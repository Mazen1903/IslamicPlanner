...
9. **Informational compound containers** (TaskCard content, PrayerHeader text block) must be grouped with `accessible={true}` and a composite `accessibilityLabel` on the container View. Interactive elements (checkboxes, buttons) must be siblings OUTSIDE the group — never nested inside. **Do NOT place `importantForAccessibility="no-hide-descendants"` on the same element that also has `accessible={true}` or another semantic role** — `no-hide-descendants` marks that element itself AND all descendants as unimportant to accessibility, which directly contradicts the intent of keeping the composite element focusable. Valid use of `no-hide-descendants`: on a native control (e.g., a `Switch`) that is intentionally suppressed from traversal because a sibling/parent wrapper View owns the full semantic role (see `Toggle.tsx` inner `Switch` pattern).
